...
1. Inspect the current repo state.
   ...
   - For Fast support, verify the source hook that computes service-tier allowance and request-tier fallback as well as the visible consumers. Settings, `/fast`, and composer Speed controls are not sufficient if `use-service-tier-settings-*.js` or an equivalent shared hook still collapses custom API users to standard, lets stale conversation-level service-tier state override Settings Fast, or lets latest-turn `params.serviceTier` from stop/edit/resend flows lock the current conversation to Standard.
