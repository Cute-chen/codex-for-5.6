...
| Composer Speed menu on `26.422.21637`, `26.422.30944`, `26.422.62136`, `26.422.71525`, `26.429.20946`, `26.429.30905`, `26.429.61741`, `26.506.21252`, `26.506.31421`, and `26.513.20950` | `Composer Intelligence Speed menu` | `index-*.js` or `composer-*.js` | `composer.intelligenceDropdown.speed.title` | Force the Intelligence dropdown speed gate to enabled so the `Speed` submenu renders. |
...
| Plugins page content on `26.429.20946`, `26.429.30905`, `26.429.61741`, `26.506.21252`, `26.506.31421`, and `26.513.20950` | `Plugins page content` | `skills-page-*.js` | `skills.pluginsAuthBlockedToast.title` | Force the `/skills` route to render the Plugins page content instead of falling back to the skills-only view for API-key users. |
| Plugin detail deep links on `26.429.20946`, `26.429.30905`, `26.429.61741`, `26.506.21252`, `26.506.31421`, and `26.513.20950` | `Plugin detail access` | `plugin-detail-page-*.js` | `pluginDeepLinkAuthBlocked` | Remove the API-key redirect that sends plugin detail deep links back to `/skills`. |
| Plugin install buttons on `26.429.20946`, `26.429.30905`, `26.429.61741`, `26.506.21252`, `26.506.31421`, and `26.513.20950` | `Plugin install availability` | `use-plugin-install-flow-*.js`, `plugins-availability-*.js`, or `check-plugin-availability-*.js` | `plugins.install.connectorUnavailable` or `connector-unavailable` | Stop the aggregate `connector-unavailable` state from disabling the top-level install action while preserving the `disabled-by-admin` block. |
| Plugin install modal details on `26.429.20946`, `26.429.30905`, `26.429.61741`, `26.506.21252`, `26.506.31421`, and `26.513.20950` | `Plugin install modal content` | `use-plugin-install-flow-*.js` or `plugins-availability-*.js` | `plugins.installModal.about` | Keep the modal's basic plugin information visible for `ON_INSTALL` app plugins instead of relying only on connector disclosure data. |
| Composer `@` plugin mentions on `26.513.20950` | `Composer plugin mentions` | `prosemirror-*.js` | `composer.atMentionList.pluginsLoading` | Remove the extra `shared-with-me` remote marketplace request so plugin mentions can use local and already available plugin results under API-key auth. |
| Browser-use native pipe peer auth on `26.513.20950` | `Browser-use native pipe peer auth` | Browser-use native pipe JS asset | `browser-use native pipe peer authorization enabled` | Wrap `authorizeSocketPeer` and only allow the `missing-code-signing-identity` rejection caused by local ad-hoc signing. Preserve every other peer-auth rejection. |
...
- `Browser-use native pipe peer auth`
  - Restore the original direct `authorizeSocketPeer` return path without the `codexfast-browser-peer-auth` wrapper
- `GPT-5.5 model list`
