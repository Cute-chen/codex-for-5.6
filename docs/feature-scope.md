...
- On `26.623.31443`, `26.623.31921`, `26.623.42026`, `26.623.61825`, `26.623.70822`, `26.623.81905`, `26.623.101652`, `26.623.141536`, and `26.707.31428`, Plugins is supported by the official app path for this repo's target use case, so `codexfast` skips Plugins runtime targets for those builds instead of applying catalog, install, mention, or app-connect patches.
...
- The same build patches the downstream model query selector because its `use_hidden_models` path applies a remote allowlist after `model/list`. The selector keeps the three GPT-5.6 ids visible and allows max/ultra through to per-model filtering; Luna still omits ultra because its model metadata does not advertise it.
